...
void processPacket(unsigned char packet[], int packet_length, int packet_address) {
    if (packet_address == CMD_BROADCAST || packet_address == deviceID || allDeviceMode) {
        while (packet_length > 0) {
            int command_size = 1;
            switch (packet[0]) {
              case (packet[0] == CMD_RESET):
                  debug("CMD_RESET\n");
                  command_size = 2;
                  deviceID = -1;
                  syncFloat();
                  break;
              case (packet[0] == CMD_SETADDRESS):
                  ...
                  syncGround();
                  break;
              case (packet[0] == CMD_READID):
                  debug("CMD_READID\n");
                  writeByte(STATUS_SUCCESS);
                  writeString("JVSE Emulator;I/O BD JVS;837-13551;Ver1.00;98/10");
                  break;
              case (packet[0] == CMD_FORMATVERSION):
                  debug("CMD_FORMATVERSION\n");
                  writeByte(STATUS_SUCCESS);
                  writeByte(0x11);
                  break;
              case (packet[0] == CMD_JVSVERSION):
                  debug("CMD_JVSVERSION\n");
                  writeByte(STATUS_SUCCESS);
                  writeByte(0x20);
                  break;
              case (packet[0] == CMD_COMMSVERSION):
                  debug("CMD_COMMSVERSION\n");
                  writeByte(STATUS_SUCCESS);
                  writeByte(0x10);
                  break;
              case (packet[0] == CMD_GETFEATURES):
                  ...
                  break;
              case (packet[0] == CMD_READSWITCHES):
                  ...
                  command_size = 3;
                  break;
              case (packet[0] == CMD_WRITEGPIO1):
                  debug("CMD_WRITEGPIO1\n");
                  command_size = 2 + packet[1];
                  writeByte(STATUS_SUCCESS);
                  break;
              case (packet[0] == CMD_WRITEGPIOBYTE):
                  debug("CMD_WRITEGPIOBYTE\n");
                  command_size = 3;
                  writeByte(STATUS_SUCCESS);
                  break;
              case (packet[0] == CMD_WRITEGPIOBIT):
                  debug("CMD_WRITEGPIOBIT\n");
                  command_size = 3;
                  writeByte(STATUS_SUCCESS);
                  break;
              case (packet[0] == CMD_SETMAINBOARDID):
              ...
              case (packet[0] == CMD_READROTARY):
                  ...
                  break;
              case (packet[0] == CMD_READSCREENPOS):
                  ...
                  break;
              case (packet[0] == CMD_WRITECOINSUBTRACT):
                  command_size = 4;
                  writeByte(STATUS_SUCCESS);
                  coin -= packet[3];
                  break;
